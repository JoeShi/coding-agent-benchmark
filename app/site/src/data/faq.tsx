/**
 * FAQ copy for `/agents/coding-agents`.
 *
 * A plain module, deliberately NOT `"use client"`: `FaqSection` is a client
 * component, and every export that crosses a `"use client"` boundary becomes a
 * client *reference* rather than the value itself — so the server page could not
 * `.map()` over it to emit the JSON-LD `FAQPage` block. Both consumers import it
 * from here instead.
 */
...
export const FAQ_ITEMS: {
  ...
  a: React.ReactNode;
  /**
   * schema.org `acceptedAnswer.text` wants a string. Only needed for the single
   * answer whose `a` is a node; the rest are strings and are used directly.
   */
  plain?: string;
}[] = [
  {
    q: "What is the Artificial Analysis Coding Agent Index?",
    a: "The Artificial Analysis Coding Agent Index is our composite score for coding-agent performance across the public benchmark suite on this page. It combines DeepSWE, Terminal-Bench v2, and SWE-Atlas-QnA to capture implementation, terminal workflow, repository-understanding, and broader software-engineering performance in a single headline metric.",
  },
  {
    q: "Which benchmarks are included in the index right now?",
    a: "The current public index includes DeepSWE, Terminal-Bench v2, and SWE-Atlas-QnA. These benchmarks are combined because they stress different parts of the coding-agent workflow rather than repeating the same task format.",
  },
  {
    q: "What kinds of tasks are these benchmarks actually testing?",
